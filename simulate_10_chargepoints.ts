// Script to simulate 10 ChargePoints (S001-S010) connecting to localhost:9000 with parallel charging sessions
...
async function main() {
  console.log("🚗 ═══════════════════════════════════════════════════════════");
  console.log("🏭 STARTING 10 CHARGEPOINT SIMULATION");
  console.log("🚗 ═══════════════════════════════════════════════════════════");
  console.log("🔌 Endpoint: ws://localhost:9000");
  console.log("📊 ChargePoints: S001 - S010");
  console.log("⏱️  Start delays: 5-15 minutes (random)");
  console.log("🔋 Charging durations: 15-30 minutes (random)");
  console.log("� OCPP Version: 1.6");
  console.log("📊 MeterValues: Energy, Power, Voltage, Current, Temperature");
  console.log("�🚗 ═══════════════════════════════════════════════════════════\n");

  // Generate configurations for 10 charge points
  const chargePointConfigs: ChargePointConfig[] = [];
  
  for (let i = 1; i <= 10; i++) {
    const chargePointId = `S${i.toString().padStart(3, '0')}`; // S001, S002, ..., S010
    const startDelay = Math.random() * (15 - 5) + 5; // 5-15 minutes
    const chargingDuration = Math.random() * (30 - 15) + 15; // 15-30 minutes
